...
class OpayGateway implements OpayGatewayCoreInterface, OpayGatewayWebServiceInterface
{
        ...
        if (!empty($this->merchantRsaPrivateKey) && !empty($this->opayCertificate)) {
            if (function_exists('openssl_pkey_get_public')) {
                ...
                if (!empty($this->signaturePassword)) {
                    ...
                    throw new OpayGatewayException('OpenSSL is not available in your server. To use RSA signature type (which is set when using setMerchantRsaPrivateKey() and setOpayCertificate()) install the OpenSSL PHP module. Otherwise use password signature (which is set when using setSignaturePassword()).',
                        OpayGatewayException::SIGNATURE_OPEN_SSL_NOT_FOUND);
                }
            }
        } else {
            if (!empty($this->signaturePassword)) {
                ...
                throw new OpayGatewayException('Signature parameters are not set. Use functions setMerchantRsaPrivateKey() and setOpayCertificate() to set parameters for RSA signature type, or setSignaturePassword() for password signature type.',
    ...
    public function signArrayOfParameters($parametersArray)
    {
        ...
        if ($signatureType == self::SIGNATURE_TYPE_RSA) {
            $parametersArray['rsa_signature'] = $this->signStringUsingPrivateKey($stringToBeSigned,
                $this->merchantRsaPrivateKey);
        } else {
            $parametersArray['password_signature'] = $this->signStringUsingPassword($stringToBeSigned,
            ...
            if (openssl_sign($stringToBeSigned, $signature, $pkeyid) === true) {
                if ($toBase64Encode == true) {
                    ...
                    if (($signature = base64_encode($signature)) !== false) {
                        ...
                        throw new OpayGatewayException('Could not encode to base64 after signing using a private key.',
                            OpayGatewayException::SIGNING_USING_PRIVATE_KEY_BASE_64_ERROR);
                    }
                } else {
                    return $signature;
                }
            } else {
                throw new OpayGatewayException('Error occurred when signing using private key',
                    OpayGatewayException::SIGNING_USING_PRIVATE_KEY_ERROR);
            }
        } else {
            throw new OpayGatewayException('Error reading private key',
                OpayGatewayException::SIGNING_USING_PRIVATE_KEY_READING_ERROR);
        }
    }
    ...
    public function generateAutoSubmitForm($url, $parametersArray, $sendEncoded = true)
    {
        ...
        $str .= '<form action="' . htmlspecialchars($url, ENT_COMPAT,
                'UTF-8') . '" method="post" accept-charset="UTF-8" name="redirectForm">';
        if ($sendEncoded == true) {
            ...
            foreach ($parametersArray as $key => $val) {
                ...
                if (!is_null($val)) {
                    ...
                    $str .= '<input type="hidden"  name="' . htmlspecialchars($key, ENT_COMPAT,
                            'UTF-8') . '" value="' . htmlspecialchars($val, ENT_COMPAT, 'UTF-8') . '" />';
                }
            }
        }
    ...
    {
        return strtr(base64_encode(http_build_query($parametersArray, '', '&', PHP_QUERY_RFC1738)),
            ['+' => '-', '/' => '_', '=' => ',']);
    }

    public function convertEncodedStringToArrayOfParameters($encodedString)
    {
        $data = strtr($encodedString, ['-' => '+', '_' => '/', ',' => '=']);
        ...
        if (($data = base64_decode($data)) !== false) {
            ...
            throw new OpayGatewayException('Base64 decoding error when converting encoded request from gateway.',
                OpayGatewayException::GATEWAY_REQUEST_BASE64_DECODE_ERROR);
        }
    }
    ...
    public function verifySignature($parametersArray)
    {
        ...
        if (!empty($rsaSignature) && !empty($this->opayCertificate)) {
            ...
            if (!empty($passwordSignature) && !empty($this->signaturePassword)) {
                return $this->verifySignatureUsingPassword($stringToBeVerified, $passwordSignature,
                    $this->signaturePassword);
            } else {
                throw new OpayGatewayException('Could not verify a signature. Signature parameters are not set properly. Use functions setMerchantRsaPrivateKey() and setOpayCertificate() to set parameters for RSA signature type, or setSignaturePassword() for password signature type. ',
                    OpayGatewayException::SIGNATURE_PARAMETERS_ARE_NOT_SET);
            }
        }
    }
        ...
        if ($pubkeyid !== false) {
            ...
            if ($ok === 1) {
                ...
                if ($ok === 0) {
                    ...
                    throw new OpayGatewayException('Error reading certificate or extracting a public key from it',
                        OpayGatewayException::SIGNATURE_VERIFICATION_USING_CERTIFICATE_ERROR);
                }
            }
        } else {
            throw new OpayGatewayException('Error reading certificate or extracting a public key from it',
                OpayGatewayException::SIGNATURE_VERIFICATION_USING_CERTIFICATE_READING_ERROR);
        }
    }
    ...
    public function webServiceRequest($url, $parametersArray, $sendEncoded = true, $decodeJson = true)
    {
        ...
        $data = $this->sendRequest($url, 'POST', $parametersArray, false,
        ...
        if ($data !== false) {
            ...
            if ($decodeJson) {
                if (version_compare(PHP_VERSION, '5.2.0', '<')) {
                    throw new OpayGatewayException('Cannot decode JSON. Your PHP version (' . PHP_VERSION . ') does not have json_decode function. This function is included starting from PHP version 5.2.0. You may pass FALSE to $decodeJson parameter when calling OpayGateway::webServiceRequest method and decode JSON by yourself.',
                ...
                if (version_compare(PHP_VERSION, '5.3.0', '>=')) {
                    ...
                    if ($jsonLastError != JSON_ERROR_NONE) {
                        throw new OpayGatewayException('Could not decode JSON. json_decode() error code is ' . $jsonLastError,
                            OpayGatewayException::JSON_DECODING_ERROR);
                    }
                } else {
                    if ($data === null) {
                        throw new OpayGatewayException('Wrong JSON format or the encoded data is deeper than the recursion limit.',
                            OpayGatewayException::JSON_DECODING_ERROR);
                    }
                }

                if (!is_array($data)) {
                    throw new OpayGatewayException('Didn\'t get an array after decoding the JSON returned by the web service',
                        OpayGatewayException::WRONG_JSON_FORMAT);
                }
            }

            return $data;
        } else {
            throw new OpayGatewayException('Could not connect to server.',
                OpayGatewayException::COMMUNICATION_WITH_SERVER_ERROR);
        }
    ...
    public function sendRequest(
        ...
        if (!array_key_exists('scheme', $parsedUrlArr)) {
            throw new OpayGatewayException('URL must contain a name of a protocol e.g. http:// or https://',
                OpayGatewayException::COMMUNICATION_WITH_SERVER_ERROR);
        }
        ...
        if (strtolower($parsedUrlArr['scheme']) == 'https') {
            ...
            if (!extension_loaded('openssl') || !in_array('https', $w)) {
                throw new OpayGatewayException('php_openssl extension have to be enabled and allow_url_fopen must be set On',
                    OpayGatewayException::COMMUNICATION_WITH_SERVER_ERROR);
            }
        }
        ...
        if ($httpMethod == 'GET') {
            ...
            $params['http']['content'] = http_build_query($parametersArray, '', '&', PHP_QUERY_RFC1738);
        }

        $params['http']['timeout'] = $timeout;
        $params['http']['header'] = "User-Agent: OPAY Client\r\n"
            . "Accept: text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8\r\n"
            . "Accept-Language: en-us,en;q=0.5\r\n"
            . "Accept-Encoding: identity\r\n" // this client does not support a compression
            . "Accept-Charset: utf-8;q=0.7,*;q=0.7\r\n"
            . "Content-Length: " . strlen($params['http']['content']) . "\r\n";
        ...
        $params['http']['header'] = rtrim($params['http']['header']);
